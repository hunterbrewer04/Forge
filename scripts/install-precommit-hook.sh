...
# Installation script for Claude Code Pre-Commit Security Hook
# Run this in your project root directory

set -e  # Exit on error

echo "🔧 Installing Claude Code Pre-Commit Security Hook..."
echo ""

# Check if we're in a git repository
if [ ! -d ".git" ]; then
  echo "❌ Error: Not a git repository. Please run this in your project root."
  exit 1
fi
...
EOF
  ...
  echo "⚠️  .claude/settings.json already exists - skipping creation"
  echo "   You may need to manually merge the hook configuration"
fi

# Create the hook script
echo "📝 Creating pre-commit security hook script..."
cat > .claude/hooks/pre-commit-security.sh << 'HOOKEOF'
#!/bin/bash
...
# Add .claude/settings.json to .gitignore if not already there
if [ -f ".gitignore" ]; then
  if ! grep -q ".claude/settings.json" .gitignore; then
    echo "📝 Adding .claude/settings.json to .gitignore..."
    echo "" >> .gitignore
...
echo "   - .claude/hooks/pre-commit-security.sh (security check script)"
echo ""
echo "🧪 To test the hook, try committing a file with a fake API key:"
echo "   echo \"const key = 'sk-1234567890abcdefghijklmnopqrstuv'\" > test.ts"
echo "   git add test.ts"
echo "   git commit -m 'test'"
echo ""
echo "   Expected: The hook should BLOCK the commit"
echo ""
echo "📖 For more information, see README-PRECOMMIT-HOOK.md"
